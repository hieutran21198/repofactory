...
1. Declare task permission `allow` only for the artifact master. Declare `deny` for each
   factory-rendered content expert. Pro: The declared config has one coordination owner. Pro: An
   absent key cannot enable a factory-rendered content expert by default. Con: All expert requests
   must pass through the artifact master.
...
`deny` for each factory-rendered content expert. Keep all task-permission declarations in
...
| F2 | Each factory-rendered content expert needs mode `subagent` and explicit task permission `deny`. OpenCode treats an absent key as default-allow. | `factory-expert` | Generate a global-settings deny for every factory-rendered content expert. Assert each mode and deny. |
...
| F7 | The evaluation needs canonical role-text checks for the feasibility route, no-subagent rule, permissions, and depth. | `factory-expert` | Add canonical role-text assertions for all four items. Add rendered-config assertions for the factory-rendered roles and depth 1. |
...
| O5 | The evaluation must inspect the permissions that the rendered config declares. | `factory-expert` | Assert `allow` for the artifact master and explicit `deny` for each factory-rendered content expert. |
...
The rendered config declares one spawning owner for the factory-rendered roles. The artifact
master can start one factory-rendered content expert when the user selects it as the primary
agent. A factory-rendered content expert cannot use an absent task key as a deny. Nix checks the
declared config and does not claim to prove OpenCode runtime behavior.
