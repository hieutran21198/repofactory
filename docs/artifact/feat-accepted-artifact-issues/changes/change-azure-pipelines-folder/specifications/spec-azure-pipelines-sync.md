...
uses the selected Azure Pipelines folder. It reuses the same synchronizer, the
same configuration, and the same notifier.
...
The factory emits `<folder>/accepted-artifact-issues.yml` only when the
project-issues composition is enabled and the CI provider is
`azure-pipelines`. The factory gets `<folder>` from
`factory.domain.ci-cd.provider.azure-pipelines.folder`. The default path is
`azure-pipelines/accepted-artifact-issues.yml`.

For the same settings, a custom folder changes only the emitted pipeline path.
The pipeline file content is byte-identical to the content with the default
folder. The factory emits no GitHub workflow file for the Azure Pipelines
selection.

The factory emits the shared scripts `.github/artifact-issues/sync.py` and
`.github/artifact-issues/notify.py`. It emits the file
`.github/artifact-issues/config.json` and the setup guides. The scripts and the
configuration are byte-identical to the GitHub Actions selection for the same
provider settings.

When the CI provider is `github-actions`, the folder option does not change a
generated path or file. The factory keeps the GitHub Actions workflow at
`.github/workflows/accepted-artifact-issues.yml` with the same content.
...
same name. The names include the project provider secrets and the notification
webhook secrets. The Telegram chat ID stays a plain variable.

## Errors

- Stop Nix evaluation before file emission when the Azure Pipelines folder
  violates `spec-ci-cd-provider`.
