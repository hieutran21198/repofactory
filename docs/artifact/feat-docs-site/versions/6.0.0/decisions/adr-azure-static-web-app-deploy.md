...
Deploy with the official mechanisms: `Azure/static-web-apps-deploy@v1` on GitHub Actions and `AzureStaticWebApp@0` on Azure Pipelines. Both use `app_location: apps/documentation`, `output_location: build`, and `skip_app_build: true`. Both read the deployment token from the configured secret.
