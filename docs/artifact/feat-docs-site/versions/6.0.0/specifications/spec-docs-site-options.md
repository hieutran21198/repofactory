...
**Covers:** req-factory-owned-site, req-generated-assets, req-azure-static-web-app
...
off by default. When it is on, the module checks the selected domains, site values, target values,
notification values, and pipeline step shapes. The module accepts the CI provider `github-actions` or the CI
provider `azure-pipelines`. Both providers share one typed step type and the same three build
hooks. Both providers support both publication targets.
...
A project that selects `github-actions` with `target = "github-pages"` keeps the behavior of version 5.0.0. A project that selects
`azure-pipelines` gets the same site project and the same typed extension values through the Azure
pipeline in [spec-docs-site-azure-pipeline](spec-docs-site-azure-pipeline.md). The target contract
is in [spec-azure-static-web-app](spec-azure-static-web-app.md).
...
The `workflow` option group keeps its name on both CI providers and on both targets. The notification option tree and
...
- `target` is `"github-pages"` or `"azure-static-web-app"`. The error message names both
  supported targets.
- When `target` is `"azure-static-web-app"`, the token secret name matches
  `[A-Za-z_][A-Za-z0-9_]*`.

All hook lists use the same assertions on both CI providers and on both targets. The error message names the docs-site
...
evaluation for an invalid CI provider, an invalid target, an invalid token secret name, an invalid path, or an invalid step relation. When `enable`
