...
Superseded by [spec-multiple-deployment-providers](spec-multiple-deployment-providers.md) for the provider selection and the secrets: the `notification.provider` option and the single webhook secret are replaced by `notification.uses` and one secret per provider. The message and the delivery contract still apply.
