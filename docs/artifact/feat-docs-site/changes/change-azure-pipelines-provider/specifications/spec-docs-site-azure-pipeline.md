...
**Covers:** req-azure-pipelines-publish, req-generated-assets, req-deployment-notification, req-multiple-deployment-providers
**Context:** context-factory
**Aggregate:** agg-repository-blueprint

## Description

The generated Azure pipeline builds the website and publishes it to GitHub Pages. It gives the
same result as the GitHub Actions workflow. It reuses the same site project, the same typed
extension values, the same pinned tooling, and the same notifier. Only the pipeline file and the
variable mapping are new.
...
pipeline also supports a manual run for setup and recovery.
...
8. Publish `apps/documentation/build` to GitHub Pages.
...
Empty extension lists add no watch path and no build step. In this state, the build and deployment
behavior match the GitHub Actions pipeline.

### Publication

The pipeline publishes the build output to GitHub Pages without a manual step. The repository
...
`.github/docs-site/notify.py` after a successful deployment. The notifier sends the same
...
Slack webhook secret, and the Telegram token secret. The Telegram chat ID stays a plain variable.

The notifier retries a network error, HTTP 429, and HTTP 5xx three times. It tries all selected
providers and then fails when one or more providers fail. When no provider is selected, the
composition adds no notifier and no notification step.

## Errors

- Stop Nix evaluation for an invalid typed value or an invalid step relation.
- Stop the build before Pages deployment when a custom step exits with a nonzero status.
- Stop notification delivery for an unsupported provider or a missing message value.
- Report the response status without the webhook URL, the bot token, or the response body.
