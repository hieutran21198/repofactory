...
task `AzureStaticWebApp@0`, `app_location: apps/documentation/build`, `output_location: build`, and
...
With `skip_app_build: true`, the task ignores `output_location`. It reads
the app artifacts directly from `app_location`. The composition keeps
