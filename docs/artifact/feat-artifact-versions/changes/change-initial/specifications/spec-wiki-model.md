# spec-wiki-model: Wiki page of the artifact-driven model

**Master:** [Specifications](README.md)
**Covers:** req-roles-follow-model, req-change-is-unit-of-work, req-version-is-full-state, req-semver-by-type
**Context:** context-factory

## Description

The wiki page of the artifact-driven model is the one place that describes the model to a person
and to an agent. The factory copies it to `docs/wiki/documentation/artifact-driven/README.md` of a
generated project. This specification gives the full replacement text of the source file
`services/factory/domain/documentation/artifact-driven/_assets/docs/wiki/documentation/artifact-driven/README.md`.
The page follows [spec-artifact-layout](spec-artifact-layout.md). The page describes a change as
the unit of work, a version as the full state, and one procedure for `change-initial` and for each
later change.

The generated copy under `docs/wiki/` is not a source. Change the source file only.

## Contract

### Full replacement text

The source file has this content:
...
### Sections that a test checks

The check in `services/factory/domain/documentation/artifact-driven/tests/eval.nix` matches these
...
- `## Where to read`
- `## The five phases`
- `### Phase 1: Requirements`, `### Phase 2: Specifications`, `### Phase 3: Plan`,
  `### Phase 4: Implementation`, `### Phase 5: Version`
- `## Rules for the artifacts of a change`
- `changes/change-initial/`, `versions/`, `templates/change/`, and `Correction`

The check fails when the source file matches one of these texts of the old model:

- `update the master artifacts`
- `Present only after a change to a feature whose code exists`
- `templates/feature/requirements`, `templates/feature/specifications`,
  `templates/feature/decisions`, or `templates/feature/tasks`
- `### Phase 5: Change`

### Module

`services/factory/domain/documentation/artifact-driven/default.nix` needs no change. The file
entry `docs/wiki/documentation/artifact-driven/README.md` copies the source file as it is.

## Errors

- The source file names a root `requirements/`, `specifications/`, `decisions/`, or `tasks/`
  folder of a feature. Remove the sentence. The check fails.
- The source file tells the author to update the master artifacts after a change. Remove the
  sentence. The check fails.
- The source file names a template under `templates/feature/` other than `README.md`. Change the
  path to `templates/change/`. The check fails.
- A generated copy under `docs/wiki/` differs from the source file. Enter the shell to render it
  again. Do not edit the copy.
