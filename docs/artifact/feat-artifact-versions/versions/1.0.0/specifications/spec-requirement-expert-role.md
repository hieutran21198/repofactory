# spec-requirement-expert-role: Requirement expert role body

**Master:** [Specifications](README.md)
**Covers:** req-roles-follow-model, req-change-is-unit-of-work, req-semver-by-type
**Context:** context-factory

## Description

The requirement expert owns phase 1 of the artifact-driven documentation model. The body of the
role is the file `services/factory/composition/artifact-driven/_assets/agent/role/requirement-expert/ROLE.md`.
...
chapter `_assets/<repo-arch>/ddd/agent/role/requirement-expert/ROLE.md` to the body.

The new body follows the layout of spec-artifact-layout. The expert has one procedure for every
change. The first build of a feature is the change `change-initial`. The expert writes the change
README for every change type. When the type is not Requirements, phase 1 is the change README
only. The expert never writes in `versions/`.

## Contract

### The role body

The file `_assets/agent/role/requirement-expert/ROLE.md` is replaced in full with this text:
...
`services/factory/composition/artifact-driven/tests/eval.nix` depends on this.

### The DDD chapter

The files `_assets/multiple/ddd/agent/role/requirement-expert/ROLE.md` and
`_assets/single/ddd/agent/role/requirement-expert/ROLE.md` change one sentence each. The
sentence under `### Procedure` anchors the DDD steps to the step of the body that writes
`requirements/README.md`. That step is step 7 in the new body.
...
The rest of each chapter is unchanged. Each chapter starts with `## Domain-Driven Design`.

### The role description

In `services/factory/composition/artifact-driven/default.nix`, the description of
`requirement-expert` in `builtinRoles` becomes this string:

```text
Gathers the business need and writes the change summary and the requirements of a feature. Owns phase 1 of the artifact-driven documentation model. Use when a new feature starts, or when a change to a feature starts.
```

The description ends with a sentence that starts with "Use".

### Names

The body uses the terms of spec-artifact-layout: change, `change-initial`, version, current
version, full replacement file, `## Removed artifacts`. The body names no root `requirements/`
folder of a feature.

## Errors

- The body names a root `requirements/` folder of a feature, for example
  `docs/artifact/feat-<name>/requirements/`. The body does not follow spec-artifact-layout.
  Correct the body.
- The body tells the expert to write under `versions/`. The body does not follow
  req-version-is-full-state. Correct the body.
- The body contains the string `expert-role`. The check `solutionExpertNamesSkill` in
...
- The anchor sentence of a DDD chapter names a step that does not write `requirements/README.md`.
  The DDD steps run before the master requirement exists. Correct the sentence.
