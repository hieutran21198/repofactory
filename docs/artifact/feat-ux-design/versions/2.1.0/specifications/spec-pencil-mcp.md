...
The Pencil MCP transport must be stdio. The pen.dev setup must supply the local
`pen-mcp-server` command. The `--app desktop` selector connects the command to the running
Pencil Desktop application. The application supplies the open `.pen` document.
...
The Claude entry must have exactly the three keys `command`, `args`, and `env`. It must contain
no `type` key.
...
{
  type = "local";
  command = [ "pen-mcp-server" "--app" "desktop" ];
  enabled = true;
}
```
...
The three entries must change together. The `command` and `args` values in the three harness
modules and in their test fixtures must use the values above. No module and no fixture must keep
the old bare `pencil` command or a partial value.
...
entry must connect to the local Pencil Desktop application for the open `.pen` document.
...
| Command | `pen-mcp-server` | `["pen-mcp-server", "--app", "desktop"]` | `pen-mcp-server` |
| Arguments | `["--app", "desktop"]` | Part of `command` | `["--app", "desktop"]` |
...
- The rendered output has the exact canonical `pen-mcp-server --app desktop` command and fields
  for the harness.
- The local check fixtures exercise the new `pen-mcp-server --app desktop` command and arguments
  together.
...
The Claude check must also prove that multiple MCP entries render in one `.mcp.json` file. The
Claude check must also assert the exact three-key set `command`, `args`, and `env`, with no
`type` key. `services/factory/composition/artifact-driven/tests/eval.nix` must prove only the
internal signal handoff. The phase 4 tasks must run all four evaluation paths.

## Description

The design-tool domain owns the passive selection. The composition owns UX Design activation.
Each harness module owns its format, merge point, and rendered file. The canonical entry runs the
local `pen-mcp-server` command with the `--app desktop` selector.

Maintainer note: the repository's own `devenv.local.nix` sets
`agent.harness.opencode.settings.mcp.pencil.environment = { }` and
`agent.harness.codex.settings.mcp_servers.pencil.env = { }`. These local values diverge from the
canonical entries: OpenCode contains no `environment` field and Codex contains no `env` field.
The canonical entries stay authoritative. This change changes no local file; the maintainer owns
the local file.
