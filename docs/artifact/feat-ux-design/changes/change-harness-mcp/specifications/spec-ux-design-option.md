...
Design template, or a design tool setting.
...
The composition must not add a harness MCP setting. Each harness module owns that setting. The
role renderer must keep its existing subagent default. The composition must keep the five phases.
...
enable value to the harness domain through one internal signal. The `use` setting is passive when
UX Design is off. Conditional output protects the byte-identical off state.
