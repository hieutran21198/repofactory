...
| 4 Implementation | Implementation experts | Code the model in the directory of the context: `services/<name>/` in the multiple repositories architecture, or `src/<name>/` in the single repository architecture. Keep the domain rules in the domain code, the use cases in the application code, and the adapters in the infrastructure code. | The code and the tests. |
