...
1. For each context that the feature touches, fill the inbound messages, the outbound
   ...
   component is one directory in `src/`.
...
4. Update `docs/domain/context-map.md` with the contract between each pair of contexts that
   communicate. Put a shared kernel or a published language in `src/<name>`.
...
- One bounded context is one directory in `src/`. A shared kernel or a published language is
  one directory in `src/` that two contexts import.
